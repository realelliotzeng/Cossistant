...
If your app already exposes the usual shadcn-style tokens on `:root` and `.dark`, the widget picks them up automatically. You do not need to mirror them into `--co-theme-*` just to get started.

The default widget looks for these host tokens:
